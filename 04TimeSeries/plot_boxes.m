...
for iDS = 1:1:numel(Settings.DataSets)
  ...
  for iYear=1:1:numel(Years)
    ...
    %pull out box and take mean (or max)
    if numel(strfind(Settings.Variables{iDS},'PW')) == 0;
      InLatRange = inrange(Data.Settings.LatScale ,Settings.LatRange{iDS});
      InLonRange = inrange(Data.Settings.LonScale ,Settings.LonRange{iDS}); 
      Data.Results = nanmean(Data.Results(:,:,:,InLonRange,InLatRange),[4,5]);
    else
      InLatRange = inrange(Data.Settings.LatScale ,Settings.LatRange{iDS});
      InLonRange = inrange(Data.Settings.LonScale ,Settings.LonRange{iDS});       
      Data.Results = nanmax(Data.Results(:,:,:,InLonRange,InLatRange),[],[4,5]);
    end
...
for iVar=1:1:numel(Settings.DataSets)
  ...
  YLim = [min(flatten(smoothn(TimeSeries(:,:,iVar),[1,Settings.SmoothDays]))), ...
          max(flatten(smoothn(TimeSeries(:,:,iVar),[1,Settings.SmoothDays])))];
  ...
  for iYear=1:1:numel(Settings.SpecialYears)
    ...
    ypos = LabelShift.*(YLim(2)-YLim(1))+YLim(2)-0.75.*(YLim(2)-YLim(1));
    if strcmp(Settings.Label{iVar},'top'); ypos = ypos + 0.1.*(YLim(2)-YLim(1)); end
  ...
  LonCentre = mean(Settings.LonRange{iVar});
  if LonCentre > 0; LonCentre = [num2str(abs(LonCentre)),'\pm',num2str(range(Settings.LonRange{iVar})./2),'E'];
  else;             LonCentre = [num2str(abs(LonCentre)),'\pm',num2str(range(Settings.LonRange{iVar})./2),'W'];
  end  
  

  
  text(datenum(2002,12,30),LabelShift.*(YLim(2)-YLim(1))+YLim(2)-0.65.*(YLim(2)-YLim(1)),[LatCentre,', ',LonCentre,' mean'],'color','k','HorizontalAlignment','right')
